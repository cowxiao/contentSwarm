...
- 按“已选/待选内容类型 → 装修场景 → 读者需求或痛点 → 有证据的核心卖点 → 标题公式 → 正文模式 → 可兑现的咨询理由”完成一次策略判断，再提交既有结构化决策。内容类型、公式和方向蓝图仍受候选与前端锁定约束；咨询理由留给成稿表达，不新增输出字段，也不凭空承诺看工地、免费方案或报价服务。
...
- 读取 `runtime_config_snapshot.price_policy`。当 `explicit_total_mode=authoritative` 且 `itemized_list_mode=may_be_partial` 时，明确投喂的总价保持原口径；分项不能核成该总价时按未穷尽清单处理，不要求补差额，不猜测缺失项目，也不声称已列分项等于总价。
...
先根据当前资料检查所有 reference_candidates 的 reference_card、structure_preview 及必要槽位，按“已准备参考选择” Skill 评分选中唯一合格最高分项，再匹配方向、公式与手法。参考必须符合当前创作类型，可借同类型下不同项目主题的结构。只返回参考 ID、source_hash、评分与必要槽位到真实事实路径的映射，不阅读全文、不提取或返回蓝图；代码按锁定资产版本读取蓝图。无合格参考时保留资料缺口，不用原文补事实。

读取 `runtime_config_snapshot.reference_policy`。当 `required_slot_mode=mapped_facts_only` 时，用户投喂数据是本项目事实来源，爆款只提供标题结构、正文层次、节奏和表达方式。当前事实已支撑方向、公式、核心主题和 `minimum_mapped_slots` 规定的参考槽位数时，选择最贴近主题的同类型参考，只映射现有事实能承接的槽位；其余信息块按 `unmapped_block_mode` 处理，不能反向变成本篇资料缺口。
